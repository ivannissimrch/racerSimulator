...
let store = {
  ...
  player_id: undefined,
  race_id: undefined,
  newNames: ['Papa Oso','Marmaduke','Donald', 'Cesar', 'Norman']
};
...
async function onPageLoad() {
	//note modify get tracks and get racers api to include images??
  try {
    ...
    getRacers().then((racers) => {
       racers.forEach((name, idx) => {      
        name.driver_name = store.newNames[idx]
...
async function runRace(raceID) {
  return new Promise((resolve) => {
    ...
    const timeInterval = setInterval(() => {getRace(raceID)
      .then(results => {
        //change names on results board
        results.positions.forEach((name, idx) => {          
           name.driver_name = store.newNames[idx]          
        });
        ...
        } else if (results.status === "finished") {
          clearInterval(timeInterval);
          renderAt('#race', resultsView(results.positions));
          resolve(results);
        }
      })},500)   
  })
...
async function getTracks() {
  ...
  try {
    ...
    console.log("error retriving tracks");
  }
}
...
async function getRacers() {
  ...
  try {
    const cars = await fetch(`${SERVER}/api/cars`);
    return cars.json();
  } catch (error) {
    console.log("error retriving tracks");
  }
...
async function createRace(player_id, track_id) {	  
  ...
  const raceData = await fetch(`${SERVER}/api/races`, {
    method: "POST",
    ...defaultFetchOpts(),
    dataType: "jsonp",
...
async function getRace(id) {
  ...
  try {
	...
	console.log('error on getRaceId function')
  }
...
async function accelerate(id) {
  ...
  try {
	...
	console.log('error on accelerate function call')
  }  
